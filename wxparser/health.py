...
def assess(hb: dict | None, cfg: Config, now: datetime | None = None) -> dict:
    ...
    # worker wedged: a backlog is queued but nothing has transcribed recently.
    if hb.get("queue_depth", 0) > 0 and (stt_age is None
                                         or stt_age > cfg.health_audio_silent_min):
        status = "degraded" if status == "ok" else status
