...
class Config:
    ...
    frequency_mhz: float = field(default_factory=lambda: float(_env("WX_FREQ_MHZ", str(get_profile()["frequency_mhz"]))))
    ...
    primary_city: str = field(default_factory=lambda: _env("WX_PRIMARY_CITY", get_profile()["primary_city"]))
    ...
    station_tz: str = field(default_factory=lambda: _env("WX_TZ", get_profile().get("tz", "America/Indiana/Indianapolis")))

    # --- Audio capture (ALSA via arecord subprocess) ---
    alsa_device: str = field(default_factory=lambda: _env("WX_ALSA_DEVICE", "plughw:0,0"))
    sample_rate: int = field(default_factory=lambda: int(_env("WX_SAMPLE_RATE", "16000")))  # whisper-native
    channels: int = 1
    ...
    capture_retry_backoff_s: float = field(default_factory=lambda: float(_env("WX_CAPTURE_BACKOFF", "1.5")))
    ...
    vad_min_silence_s: float = field(default_factory=lambda: float(_env("WX_VAD_MIN_SILENCE", "1.0")))
    vad_min_speech_s: float = field(default_factory=lambda: float(_env("WX_VAD_MIN_SPEECH", "1.0")))
    vad_max_segment_s: float = field(default_factory=lambda: float(_env("WX_VAD_MAX_SEGMENT", "28")))
    ...
    fp_similarity_threshold: float = field(default_factory=lambda: float(_env("WX_FP_SIMILARITY", "0.97")))
    ...
    whisper_dynamic_audio_ctx: bool = field(default_factory=lambda: _env("WX_AUDIO_CTX", "1") == "1")
    ...
    whisper_prompt_max_ctx: int = field(default_factory=lambda: int(_env("WX_PROMPT_MAX_CTX", "64")))
    ...
    stt_enhance_mains_hz: float = field(default_factory=lambda: float(_env("WX_STT_ENHANCE_MAINS_HZ", "60")))
    stt_enhance_lowpass_hz: float = field(default_factory=lambda: float(_env("WX_STT_ENHANCE_LOWPASS_HZ", "3800")))
    stt_enhance_alpha: float = field(default_factory=lambda: float(_env("WX_STT_ENHANCE_ALPHA", "2.0")))
    stt_enhance_floor: float = field(default_factory=lambda: float(_env("WX_STT_ENHANCE_FLOOR", "0.12")))
    ...
    repetition_unique_ratio: float = field(default_factory=lambda: float(_env("WX_REP_UNIQUE_RATIO", "0.35")))
    ...
    stt_confidence_floor: float = field(default_factory=lambda: float(_env("WX_STT_CONF_FLOOR", "0.5")))
    ...
    text_dup_threshold: float = field(default_factory=lambda: float(_env("WX_TEXT_DUP", "0.97")))     # >= -> duplicate, drop
    text_update_threshold: float = field(default_factory=lambda: float(_env("WX_TEXT_UPDATE", "0.75")))  # >= same-type -> update
    ...
    health_audio_silent_min: int = field(default_factory=lambda: int(_env("WX_HEALTH_AUDIO_SILENT_MIN", "5")))
    health_heartbeat_stale_min: int = field(default_factory=lambda: int(_env("WX_HEALTH_HEARTBEAT_STALE_MIN", "3")))
    ...
    health_stt_wedged_min: int = field(default_factory=lambda: int(_env("WX_HEALTH_STT_WEDGED_MIN", "10")))
    health_stt_wedged_queue: int = field(default_factory=lambda: int(_env("WX_HEALTH_STT_WEDGED_QUEUE", "1")))
    ...
    health_novel_stale_min: int = field(default_factory=lambda: int(_env("WX_HEALTH_NOVEL_STALE_MIN", "60")))
    ...
    webhook_timeout_s: float = field(default_factory=lambda: float(_env("WX_WEBHOOK_TIMEOUT", "5")))
    ...
    condition_stale_after_min: int = field(default_factory=lambda: int(_env("WX_STALE_AFTER_MIN", "60")))
    ...
    almanac_stale_after_min: int = field(default_factory=lambda: int(_env("WX_ALMANAC_STALE_AFTER_MIN", "1440")))
    ...
    trust_sightings_full: float = field(default_factory=lambda: float(_env("WX_TRUST_SIGHTINGS_FULL", "6")))
    ...
    alert_link_pre_buffer_s: int = field(default_factory=lambda: int(_env("WX_ALERT_LINK_PREBUFFER", "120")))
    ...
    alert_priority_window_s: float = field(default_factory=lambda: float(_env("WX_ALERT_PRIORITY_WINDOW", "120")))
    ...
    pg_password: str = field(default_factory=lambda: _env("WX_PG_PASSWORD", ""))  # empty -> trust auth (no password)
