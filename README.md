...
| `WX_STT_MAX_QUEUE` | `4` | STT **backpressure** cap: the producer sheds a routine segment once the queue holds this many. This is the real load limit — the fingerprint can't be, since it can't tell content apart. Alert narratives never shed. Keeps conditions fresh (every segment flows when STT has room) without the runaway a high similarity threshold caused |
